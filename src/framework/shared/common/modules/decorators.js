...
export function interfaceModel(Target, key, descriptor) {
    if (!Target._config) {
        throw new IException('interfaceModel must implement static get _config function return interface config')
    }

    const interfaceConfig = Target._config
...
export function dbModel(Target, key, descriptor) {
    const db = Target._db
